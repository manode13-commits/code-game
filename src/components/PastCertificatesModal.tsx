...
import { ConnectionStatus } from '../services/firebase';
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        {/* Firebase Cloud Firestore Connection Bar */}
        <div className="bg-slate-900 text-slate-200 px-4 py-2 flex flex-wrap items-center justify-between gap-2 text-[11px] border-b border-slate-800">
          <div className="flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${dbStatus?.online ? 'bg-emerald-400 animate-pulse' : dbStatus ? 'bg-amber-400' : 'bg-slate-500'}`} />
            <span className="font-bold text-white flex items-center gap-1.5">
              <Database className="w-3.5 h-3.5 text-emerald-400" />
              <span>{dbStatus?.online ? 'เชื่อมต่อ Cloud Firestore แล้ว' : 'สถานะฐานข้อมูล: ออฟไลน์ / ใช้แคชในตัว'}</span>
            </span>
          ...
          <button
            ...
            disabled={testingDb}
            className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-emerald-300 border border-slate-700 rounded-lg font-bold text-[10.5px] flex items-center gap-1.5 transition cursor-pointer disabled:opacity-50"
          >
            <RefreshCw className={`w-3 h-3 ${testingDb ? 'animate-spin text-emerald-400' : 'text-emerald-400'}`} />
            <span>{testingDb ? 'กำลังทดสอบ Ping...' : 'ทดสอบเชื่อมต่อ Firestore'}</span>
          </button>
        </div>
